...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        hidden=X_reshaped.dot(self.params['W1'])+self.params['b1']
...
class FullyConnectedNet(object):
        ...
        for i in range (len(self.hidden_dims)+1):
          if i==len(self.hidden_dims):
            ...
            if self.normalization=="batchnorm":
              gamma=np.ones(next_dim,)
              beta=np.zeros(next_dim,)
              self.params["gamma"+str(i+1)]=gamma
              self.params["beta"+str(i+1)]=beta
          W=np.random.randn(prev,next_dim)*self.weight_scale
          b=np.zeros(next_dim,)
    ...
    def loss(self, X, y=None):
        ...
        scores,cache[L+1]=affine_forward(out, W, b)
        ...
        dout,dW,db=affine_backward(dscores,cache[L+1])
